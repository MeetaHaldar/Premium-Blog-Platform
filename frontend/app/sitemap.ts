...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages: MetadataRoute.Sitemap = [
    ...
    { url: `${siteUrl}/blogs`, lastModified: new Date(), changeFrequency: 'hourly', priority: 0.9 },
  ...
  // Dynamic blog pages
  try {
    const res = await fetch(`${API_URL}/blogs?limit=100&status=published`, { next: { revalidate: 3600 } });
